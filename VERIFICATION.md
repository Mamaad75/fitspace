...
- Domain integration: 20 tests, actual SQL migrations and service/API code.
- Standalone HTTP, backup and migration integration: 3 tests, real compiled application handler.
...
Class coverage includes room/trainer conflicts, cross-tenant and member access, reservation retries, waitlist promotion, inactive/expired eligibility, overlapping member bookings, capacity constraints, class cancellation, attendance timing and ICS escaping. The standalone HTTP test sends two concurrent requests for one seat and confirms exactly one booking plus one waitlist entry.

Upgrade coverage starts with the three 0.2 SQL migrations, retains an existing gym row, then applies the two new migrations and verifies the class tables. Waitlist tests also revalidate eligibility after a candidate becomes inactive.
