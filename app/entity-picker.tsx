...
export default function EntityPicker({api,table,value,onChange,initial=[],memberId='',required=false}:{api:(path:string)=>Promise<any>,table:string,value:string,onChange:(v:string)=>void,initial?:Item[],memberId?:string,required?:boolean}){
 ...
 useEffect(()=>{let active=true;const timer=setTimeout(()=>{api('page/'+table+'?'+new URLSearchParams({q:query,limit:'30',...(memberId?{member_id:memberId}:{})})).then(r=>{if(active){setItems(r.items);setError('')}}).catch(e=>{if(active)setError(e.message)})},250);return()=>{active=false;clearTimeout(timer)}},[api,query,table,memberId]);
